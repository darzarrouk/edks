...
""" %(dR, Rmax, deps)
   ...
   IDs = [str(NP.random.random_integers(5)) for i in range(0, len(xS))]
   IDs.sort()
   EUxS,EUyS,EUzS =layered_disloc_sub(IDs, xS*km, yS*km, zS*km, strike, dip, rake, slip,\
                 aS*km*km, xR*km, yR*km, 'test_1a.edks', 'test_1a') 
   # I have to add the different subfaults  
   sumDir = 1
   EUx = NP.sum(EUxS, sumDir)
   EUy = NP.sum(EUyS, sumDir)
   EUz = NP.sum(EUzS, sumDir)
