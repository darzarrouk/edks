...
prefix = 'example'; %prefix to be used for IO matlab/fortran
...
temp = [xs(:), ys(:), zs(:), strike(:), dip(:), rake(:), ...
	W(:), L(:), slip(:)]';
fid  = fopen(file_pat,'wb');
fwrite(fid, temp, 'real*4');
fclose(fid);

%%%%% call sum_layered to calculate Greens functions

comstr = [BIN '/sum_layered ' edks ' ' prefix ' ' int2str(nrec) ' ' int2str(np) ' ' int2str(npw) ' ' int2str(npy)]
unix(comstr);

%%%%% read sum_layered output Greens function

fid  = fopen(file_dux,'rb');
[ux, count] = fread(fid, [nrec, np], 'real*4');
fclose(fid);

fid  = fopen(file_duy,'rb');
[uy, count] = fread(fid, [nrec, np], 'real*4');
fclose(fid);

fid  = fopen(file_duz,'rb');
