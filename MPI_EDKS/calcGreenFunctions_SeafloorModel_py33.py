...
def interpTri2Rec(IDs, xs, ys, slip, A, xr, yr):
   ...
   us = NP.zeros((max(NumSubSources.values()), 3))
   ur = NP.zeros(len(xr))
   k = 0
   for ind, j in enumerate(sortedListOfFiniteFaultIDs):
      ...
      # print us
      if ind==2:
         NP.savetxt('test-ind2.txt', us)
      xmin = us[0:NumSubSources[j], 0].min()
      xmax = us[0:NumSubSources[j], 0].max()
      ymin = us[0:NumSubSources[j], 1].min()
      ymax = us[0:NumSubSources[j], 1].max()
      log0.addLine('before mask')
      mask = (xr > (xmin-10)) & (xr < (xmax+10)) & (yr > (ymin-10)) & (yr < (ymax+10))
      ...
      ur[mask] = interpolate.griddata(NP.vstack((us[0:NumSubSources[j],0], us[0:NumSubSources[j],1])).T, 
                                      us[0:NumSubSources[j],2], NP.vstack((xr[mask], yr[mask])).T, 
                                      )
      log0.addLine('after interp')
      if ind==2:
         NP.savetxt('test-ur.txt', ur)
      u[:, ind] = ur/ur.max()
      k = k + NumSubSources[j]
