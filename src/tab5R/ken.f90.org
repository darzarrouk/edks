...
!	print *, n, m
	...
	!               when called with n=0; only 'Ruac' is 
		...
		call ERTUDPSV(m, k, Emm1,Rum,Tum,Rdm,Tdm)
	        ...
	        Tdac  =      matmul(matmul(Tdbc,inv22(Id2-matmul(Ruab,Rdbc))),Tdab)
	        Rdac  = Rdab+matmul(matmul(matmul(Tuab,Rdbc),inv22(Id2-matmul(Ruab,Rdbc))),Tdab)
	        Tuac  =      matmul(matmul(Tuab,inv22(Id2-matmul(Rdbc,Ruab))),Tubc)
	        Ruac  = Rubc+matmul(matmul(matmul(Tdbc,Ruab),inv22(Id2-matmul(Rdbc,Ruab))),Tubc)
	endif
