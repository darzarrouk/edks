...
xs     = [375]; %middle of top edge of fault
ys     = [0]; %middle of top edge of fault
zs     = [1.0e2]; %middle of top edge of fault
...
[uxt, uyt, uzt] = layered_disloc2(xs, ys, zs, strike, dip, rake, slip, ...
...
for ip = 1:np
  ...
  subplot(3,2,6)
  ax1=plot(xr,uzp,'o',xr,uzop,'+'); hold on
  ax2=plot(xr,uzp,xr,uzop);
  ax3=plot(xr,(uzp-uzop)/max(abs(uzop(:)))*100); hold off
  grid on 
  orient tall
  wysiwyg
